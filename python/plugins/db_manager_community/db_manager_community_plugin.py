...
from qgis.core import QgsApplication, QgsDataSourceUri, QgsMapLayerType, QgsProject, QgsSettings
...
class DBManagerCommunityPlugin:
    def __init__(self, iface):
        ...
        # when community plugin is installed, disable the inbuilt deprecated plugin
        QgsSettings().setValue(f"/PythonPlugins/db_manager", False)

    @staticmethod
    def unload_plugin(plugin_name: str):
        """
        Force unloads another running plugin
        """
        if plugin_name in qgis_utils.plugins:
            qgis_utils.unloadPlugin(plugin_name)

    def initGui(self):
        # on startup, force unload the inbuilt deprecated plugin
        self.unload_plugin('db_manager')
