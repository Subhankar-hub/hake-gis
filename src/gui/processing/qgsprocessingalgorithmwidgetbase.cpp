  ...
  , mMode( mode )
{
  ...
  switch ( mMode )
  {
    case QgsProcessingAlgorithmWidgetBase::WidgetMode::Single:
    {
      ...
      mCopyAsQgisProcessCommand = new QAction( tr( "Copy as hake-geodesk-process Command" ), mAdvancedMenu );
      ...
      connect( mCopyAsQgisProcessCommand, &QAction::triggered, this, [this] {
        if ( const QgsProcessingAlgorithm *alg = algorithm() )
        {
          ...
          if ( !ok )
          {
            mMessageBar->pushMessage( tr( "Current settings cannot be specified as arguments to hake-geodesk-process (Pipe parameters as JSON to hake-geodesk-process instead)" ), Qgis::MessageLevel::Warning );
          }
          else
          {
            QMimeData *m = new QMimeData();
