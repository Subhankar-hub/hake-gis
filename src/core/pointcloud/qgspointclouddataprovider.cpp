...
QgsPointCloudDataProvider::QgsPointCloudDataProvider( const QString &uri, const QgsDataProvider::ProviderOptions &options, Qgis::DataProviderReadFlags flags )
...
struct MapIndexedPointCloudNode
{
    ...
    MapIndexedPointCloudNode(
      ...
      , mExtentGeometry( extentGeometry )
      , mZRange( zRange )
      , mIndex( std::move( index ) )
      , mPointsLimit( pointsLimit )
    {}
...
QVector<QVariantMap> QgsPointCloudDataProvider::identify( double maxError, const QgsGeometry &extentGeometry, const QgsDoubleRange &extentZRange, int pointsLimit )
{
  QVector<QVariantMap> acceptedPoints;
  ...
  for ( QgsPointCloudSubIndex &subidx : subIndexes() )
  {
    ...
    if ( !index || ( !subidx.zRange().overlaps( extentZRange ) ) || !subidx.polygonBounds().intersects( extentGeometry ) )
      continue;
...
QVector<QVariantMap> QgsPointCloudDataProvider::identify( QgsPointCloudIndex &index, double maxError, const QgsGeometry &extentGeometry, const QgsDoubleRange &extentZRange, int pointsLimit )
{
  QGIS_PROTECT_QOBJECT_THREAD_ACCESS
